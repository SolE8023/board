...
async function editRequest(password, type) {
    if (await checkPassword(password, type)) location.href = `/calendar/edit/${popupId}`
}
...
async function deleteRequest(password, type) {
    ...
    const url = `/calendar/delete/${postId}`
...
async function viewRequest(password, type) {
    popupId = getId()
    if (await checkPassword(password, type)) location.href = `/calendar/view/${popupId}`
}

async function checkPassword(password, type) {
    const url = `/calendar/chkPwd`
    const options = {
        method: 'POST',
        headers: {
